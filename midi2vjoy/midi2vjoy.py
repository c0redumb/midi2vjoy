    ...
    with open(conf_file, 'r') as f:
        for l in f:
            if len(l.strip()) == 0 or l[0] == '#':
                continue
            fs = l.split()
            # we encountered a pitch wheel.
            if(int(fs[0]) & 0xE0 == 0xE0):
                # so the key is just the pitch wheel number
                key = int(fs[0])
            else:
    ...
    try:
        ...
        while True:
            while midi.poll():
                ...
                reading = ipt[0][0][2]                                
                print(key, reading)   
                # we encountered a pitch wheel event
                if (key[0] & 0xE0) == 0xE0:
                    if options.verbose:
                        print('pitchWheel: ', key[0])     
                    # Check that the input is defined in table                        
                    if not key[0] in table:
                        continue
                    # so key is now not the tuple but only the 0xEx 
                    opt = table[key[0]]
                    if options.verbose:
                        print(key[0], '->', opt, reading)
                    # handle as axis.
                    reading = (reading + 1) << 8
